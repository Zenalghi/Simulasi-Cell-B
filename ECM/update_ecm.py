...
for i in range(1, len(pulse_starts), 2):
    ...
    try:
        ...
        ecm_parameters.append([soc_terkini, R0_ohms, R1_ohms, C1_farads])
    except RuntimeError:
        pass

df_ecm = pd.DataFrame(ecm_parameters, columns=['SOC', 'R0', 'R1', 'C1'])
df_ecm = df_ecm.sort_values(by='SOC', ascending=True).reset_index(drop=True)

# Format to match the previous CSV exactly
df_ecm['SOC'] = df_ecm['SOC'].apply(lambda x: f"{x:.6f}")
df_ecm['R0'] = df_ecm['R0'].apply(lambda x: f"{x:.6f}")
df_ecm['R1'] = df_ecm['R1'].apply(lambda x: f"{x:.6f}")
df_ecm['C1'] = df_ecm['C1'].apply(lambda x: f"{x:.2f}")

out_path = r"c:\Users\zenaj\Documents\Courses\Sms 8\Simulasi-Cell-B\Simulasi Proccessor In the Loop\pil-soc\battery_model_ecm.csv"
df_ecm.to_csv(out_path, index=False)
print("Updated ECM model written to:", out_path)
