"""
Step-by-step EKF trace for first 20 samples of charge_rest_60m
to understand why ALL strategies diverge on this dataset.
"""
import csv, math
...
def get_docv(soc):
    soc = max(0,min(1,soc)); h=0.005
    lo,hi = max(soc-h,0), min(soc+h,1)
    ds = hi-lo
    return (get_ocv(hi)-get_ocv(lo))/ds if ds > 1e-6 else 0
...
soc_true = 0.0
soc_algo = 0.10  # 10% offset
ekf_x = [soc_algo, 0.0]
ekf_P = [[1.0, 0.0], [0.0, 0.1]]
Q00, Q11 = 1e-5, 1e-4

print(f"{'Step':>4} {'t':>5} {'I':>6} {'V':>6} | {'SOC_true':>8} {'SOC_ekf':>8} {'err':>7} | {'OCV':>6} {'V_pred':>7} {'innov':>7} | {'dOCV':>6} {'R_dyn':>8} {'K0':>8} {'K1':>8}")
print("-"*130)
...
for step, (t, I, V) in enumerate(rows):
    ...
    if step == 0:
        print(f"{step:4d} {t:5.0f} {I:6.2f} {V:6.3f} | {soc_true:8.4f} {ekf_x[0]:8.4f} {(soc_true-ekf_x[0])*100:7.2f}% | {'INIT':>6} {'':>7} {'':>7} | {'':>6} {'':>8} {'':>8} {'':>8}")
        continue
    
    soc_true = max(0, min(1, soc_true - I*dt/Q_COULOMB))
    
    soc_prev = max(0, min(1, ekf_x[0]))
    vc1_prev = ekf_x[1]
    ...
    soc_pred = max(0, min(1, soc_prev - I*dt/Q_COULOMB))
    ...
    P_pred = [[ekf_P[0][0]+Q00, ekf_P[0][1]*alpha],
              [ekf_P[1][0]*alpha, alpha*alpha*ekf_P[1][1]+Q11]]
    ...
    h0, h1 = dOCV, -1.0
    R_dyn = 0.0005 / (abs(dOCV) + 1e-4)
    
    S = h0*h0*P_pred[0][0] + h0*h1*P_pred[0][1] + h1*h0*P_pred[1][0] + h1*h1*P_pred[1][1] + R_dyn
    K = [(P_pred[0][0]*h0+P_pred[0][1]*h1)/S, (P_pred[1][0]*h0+P_pred[1][1]*h1)/S]
    
    innov = V - V_pred
    ekf_x[0] = max(0, min(1, ekf_x[0] + K[0]*innov))
    ekf_x[1] += K[1]*innov
    
    # Joseph form
    IKH = [[1-K[0]*h0, -K[0]*h1], [-K[1]*h0, 1-K[1]*h1]]
    T = [[IKH[0][0]*P_pred[0][0]+IKH[0][1]*P_pred[1][0], IKH[0][0]*P_pred[0][1]+IKH[0][1]*P_pred[1][1]],
         [IKH[1][0]*P_pred[0][0]+IKH[1][1]*P_pred[1][0], IKH[1][0]*P_pred[0][1]+IKH[1][1]*P_pred[1][1]]]
    ekf_P[0][0] = T[0][0]*IKH[0][0]+T[0][1]*IKH[0][1]+K[0]*K[0]*R_dyn
    ekf_P[0][1] = T[0][0]*IKH[1][0]+T[0][1]*IKH[1][1]+K[0]*K[1]*R_dyn
    ekf_P[1][0] = T[1][0]*IKH[0][0]+T[1][1]*IKH[0][1]+K[1]*K[0]*R_dyn
    ekf_P[1][1] = T[1][0]*IKH[1][0]+T[1][1]*IKH[1][1]+K[1]*K[1]*R_dyn
    ekf_P[0][1]=(ekf_P[0][1]+ekf_P[1][0])*0.5; ekf_P[1][0]=ekf_P[0][1]
    ekf_P[0][0]=max(ekf_P[0][0],1e-10); ekf_P[1][1]=max(ekf_P[1][1],1e-10)
    
    print(f"{step:4d} {t:5.0f} {I:6.2f} {V:6.3f} | {soc_true:8.5f} {ekf_x[0]:8.5f} {(soc_true-ekf_x[0])*100:7.2f}% | {OCV:6.3f} {V_pred:7.3f} {innov:7.3f} | {dOCV:6.2f} {R_dyn:8.5f} {K[0]:8.5f} {K[1]:8.5f}")
