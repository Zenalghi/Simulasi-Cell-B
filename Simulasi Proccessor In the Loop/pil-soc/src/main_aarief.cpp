...
void runEKFStep(float I_meas, float V_meas, float dt)
{
  ...
  P_pred[0][1] = 0.0f; // Decoupled for float32 stability on LiFePO4 flat plateau
  P_pred[1][0] = 0.0f; // Decoupled for float32 stability on LiFePO4 flat plateau
  ...
  // Cap Vc1 to prevent numerical explosion (physically bounded by I*R1)
  if (ekf_x[1] > 0.5f)
    ekf_x[1] = 0.5f;
  if (ekf_x[1] < -0.5f)
    ekf_x[1] = -0.5f;
